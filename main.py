...
@app.route('/breastcancer_ask',methods=['GET','POST'])
def breastcancer_ask():
...
# Diabetes section
@app.route('/diabetes_ask',methods=['GET','POST'])
def diabetes_ask():
    if request.method == "POST":
        try:
...
@app.route('/heart_ask', methods=['POST'])
def heart_ask():
    # Get form data
    ...
    chestpain_dict = {'ATA': 0, 'NAP': 1, 'ASY': 2, 'TA': 3}
    chestpain = chestpain_dict[chestpain]
    restingecg_dict = {'Normal': 0, 'ST': 1, 'LVH': 2}
    restingecg = restingecg_dict[restingecg]
    exercise = 1 if exercise == 'Y' else 0
    stslope_dict = {'up': 0, 'flat': 1, 'down': 2}
    stslope = stslope_dict[stslope]

    # Create feature array for prediction
    features = np.array([[age, sex, chestpain, restingbp, cholestrol, fastingbs, restingecg, maxhr, exercise, oldpeak, stslope]])

    # Predict using the model
    model = joblib.load('heart_failure_model.pkl')
    prediction = model.predict(features)
    prediction_text = 'Heart Failure Detected' if prediction[0] == 1 else 'No Heart Failure'

    return jsonify({'predict': prediction_text})
...
# model = load_model(PNEUMONIA_MODEL_PATH,compile=False)
# # pneumonia detection
